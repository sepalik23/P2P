...
# 1 "/home/charliyan23/OLSONET/PICOS/PicOS/PLibs/VNetI/tcvplug.h"
...
# 19 "/home/charliyan23/OLSONET/PICOS/PicOS/PLibs/VNetI/tcvplug.h"
...
const tcvplug_t plug_null =
  { tcv_ope_null, tcv_clo_null, tcv_rcv_null, tcv_frm_null,
   tcv_out_null, tcv_xmt_null, 0,
    0x0001 };

static int *ndsc_null = 0;
...
static int tcv_ope_null (int phy, int fd, va_list plid) {
 ...
 if (ndsc_null == 0) {
  ndsc_null = (int*) ((address)__pi_malloc (sizeof (int) * 3));
  if (ndsc_null == 0)
   __pi_syserror (6, "plug_null tcv_ope_null");
  for (i = 0; i < 3; i++)
   ndsc_null [i] = (-1);
 }
...
static int tcv_clo_null (int phy, int fd) {
 ...
 if (ndsc_null == 0 || ndsc_null [phy] != fd)
  return (-1);
...
static int tcv_rcv_null (int phy, address p, int len, int *ses,
 ...
 if (ndsc_null == 0 || (*ses = ndsc_null [phy]) == (-1))
  return 0;
